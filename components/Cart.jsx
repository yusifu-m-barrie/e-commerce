...
const Cart = () => {
  ...
  const generateWhatsAppLink = () => {
    ...
    const message = cartItems.map(item => `I am in need of ${item.quantity} ${item.name} (NLE${item.price}) - ${urlFor(item?.image[0])}`).join('\n') + `\n\nTotal: NLE${totalPrice}.`;
